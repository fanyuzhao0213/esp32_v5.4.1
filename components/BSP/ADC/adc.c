...
/*
LOST_VAL 定义为 1，表示丢弃最高和最低的1个采样值（两端的极值）。
这一步剔除采样中可能的异常极端值（噪声尖峰或偶发干扰）。
*/
#define LOST_VAL    1
...
void adc_task(void *pvParameters)
{
    ...
    while (1)
    {
        adcdata = adc_get_result_average(ADC_CHAN, 10);
        voltage = (float)adcdata * (3.3 / 4096);            /* 获取计算后的带小数的实际电压值 */
        // ESP_LOGI("ADC", "原始 adcdata :%d",adcdata);
        // ESP_LOGI("ADC", "转换 voltage:%.2f",voltage);
        vTaskDelay(pdMS_TO_TICKS(2000));                    // 延时等待下一次采样
    }
}
